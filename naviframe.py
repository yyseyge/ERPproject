...
class NaviFrame(tk.Frame):
    def __init__(self, root,tableData, target, x=700, y=180, width=602):
        ...
        def confirmBtn():
            if self.dataTable.checked_data():
    ...
    def insertData(self, targetList):
        if targetList and self.checkedData:
            if targetList[0].get():
                targetList[0].delete(0,tk.END)
                targetList[0].insert(0,self.checkedData[0])
            else:
                targetList[0].insert(0, self.checkedData[0])
        else:
            pass
